...
Store the signal in array `x` with sampling rate `fs` (in Hz) as a `waveform` object.

The element type of `x` (any `AbstractFloat`) sets the numeric precision used by all
subsequent processing of the waveform. If `x` is a matrix it is assumed to hold one channel
...
function waveform(x::AbstractArray{T}, fs::Number) where {T<:AbstractFloat}
    if(ndims(x)>2)
    ...
    return waveform(signal,convert(T,fs))
end


"""
    framed_signal(sig::waveform, win_dur=0.02, win_shift=0.01)
    framed_signal(x, fs, win_dur=0.02, win_shift=0.01)

Describe a division of the signal in the [`waveform`](@ref) `sig` (or in array `x` with
sampling rate `fs`) into frames of duration `win_dur` seconds whose start points are
`win_shift` seconds apart. No copies of the signal are made: the object stores the waveform
plus the framing geometry, and frames are materialised on demand by [`view_frame`](@ref),
[`extract_frame`](@ref) or [`enframe`](@ref).

# Fields
- `signal::waveform{T}` : the underlying signal
...
function framed_signal(signal::waveform,win_dur::Number=0.02,win_shift::Number=0.01)
    fs = signal.fs
    x = signal.x
...
Hold the spectral `components` (real or complex, matching the precision of `signal`) of the
[`waveform`](@ref) `signal`, where component `i` corresponds to the frequency `frqs[i]` in
...
# Fields
- `signal::waveform{T}` : the analysed signal
...
function spectrum(signal::waveform{T}, components::AbstractVector{S}, frqs::AbstractVector{<:Real}, title::AbstractString) where {T<:AbstractFloat, S<:Union{T,Complex{T}}}
    return spectrum(signal, convert(Vector{S},components), convert(Vector{T},frqs), title)
end

spectrum(signal::waveform, components::AbstractVector, frqs::AbstractVector{<:Real}) = spectrum(signal,components,frqs,"")


"""
    timefreq(signal, frames, components, frqs, time, title)
    timefreq(signal, components, frqs, time, title)
...
The representation is anchored to the [`waveform`](@ref) `signal` it was computed from and,
...
function timefreq(signal::waveform{T},
        frames::Union{framed_signal{T}, Nothing},
